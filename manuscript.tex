...
When processing Rayonix images with $1920 \times 1920$ pixels, \peaknet{} achieved a significantly reduced processing time of 90 milliseconds per event on an NVIDIA 1080 Ti GPU for peak finding and auto-mask generation.  In contrast, \psocake{} required 176 milliseconds per event when executed on an Intel E5-2620 CPU and more than 350 milliseconds for Cheetah \citep{hadian-jaziPeakfindingAlgorithmBased2017} and 120 milliseconds for Robust Peak Finder (RBF) on 3 times smaller AGIPD 1M images \citep{hadian-jaziDataReductionSerial2021}. Note that computation time will be even longer if auto-masking feature is introduced to these existing peak finders.  This result clearly demonstrates that \peaknet{} not only delivers robust Bragg peak finding results in an autonomous fashion, but also exhibits remarkable runtime efficiency.  %This establishes it as a highly competent tool for real-time autonomous Bragg peak finding.
...
\caption{
    ...
    hadian-jaziDataReductionSerial2021} and Peakfinder8 . RPF runtime performance was originally measured
...
Peakfinder8     &  No          & Intel E5-2698  & 1024 $\times$ 1024 & $\approx$ RPF        \\ \hline
